...
/*	@ExceptionHandler(TypeMismatchException.class)
	...
	/**
	 * 数据回显将数据存在request域中，带到页面。
	 * @param request
	 * @param ex
	 * @return
	 */
	@ExceptionHandler(TypeMismatchException.class)
	public ModelAndView exceptionResovler(HttpServletRequest request, Exception ex){
		ModelAndView mv = new ModelAndView();
		...
		}else if(msg.contains(birthday)){
			mv.addObject("birthdayErrors", "生日日期输入有误");
		}
		
		
		mv.addObject("age", age);//相当于存在request域中
		mv.addObject("birthday", birthday);
		mv.addObject("ex",ex);
